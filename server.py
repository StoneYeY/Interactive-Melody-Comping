...
def inspect_vocabulary():
    ...
    if note2in:
        ...
        print(f"   音符词汇表示例 (键->值): {dict(list(note2in.items())[:5])}")
        print(f"   音符键类型: {type(note_keys[0]) if note_keys else 'empty'}")
        print(f"   音符值类型: {type(note_values[0]) if note_values else 'empty'}")

    if chord2in:
        chord_keys = list(chord2in.keys())[:10]
        chord_values = list(chord2in.values())[:10]
        print(f"   和弦词汇表示例 (键->值): {dict(list(chord2in.items())[:5])}")
        print(f"   和弦键类型: {type(chord_keys[0]) if chord_keys else 'empty'}")
        print(f"   和弦值类型: {type(chord_values[0]) if chord_values else 'empty'}")

    if in2note:
        print(f"   反向音符词汇表示例: {dict(list(in2note.items())[:5])}")

    if in2chord:
        print(f"   反向和弦词汇表示例: {dict(list(in2chord.items())[:5])}")
...
def load_model():
    ...
    try:
        ...
        print(f"   音符词汇量: {len(note2in)}")
        print(f"   和弦词汇量: {len(chord2in)}")
        ...
        if not os.path.exists(model_path):
            ...
            if not os.path.exists(model_path):
                raise FileNotFoundError("找不到模型文件，请确保模型已训练并保存")
...
def api_harmonize():
    ...
    try:
        ...
        if mode == 'notes':
            ...
            print(f"✅ 生成成功，返回结果: {response_data}")
...
if __name__ == '__main__':
    print("🚀 启动定制化 AI 音乐服务器...")
    print("📍 端点:")
    print("  GET  /api/status     - 健康检查")
    print("  POST /api/harmonize  - 和弦生成")
    print("🌐 服务器地址: http://localhost:5001")
    print("🔧 CORS: 已启用，允许所有来源")
    print("=" * 60)
    ...
    if model_loaded:
        print("🎉 服务器准备就绪，使用完整的自定义 Transformer 模型!")
    else:
        print("⚠️  模型加载失败，将使用简化版作为备用")

    print("=" * 60)# backend/server.py
